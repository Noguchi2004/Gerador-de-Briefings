...
import { baixarArquivoMarkdown, baixarArquivoCsv, baixarDocxCronograma } from './utils/exportUtils';
import { Sparkles, Sliders, FileDown, Loader2, CheckCircle2, X } from 'lucide-react';
...
export default function App() {
  ...
  const [bannerGerado, setBannerGerado] = useState<boolean>(false);
  ...
  // Download do Briefing Completo em DOCX
  const handleBaixarDocxCompleto = async () => {
    try {
      setIsDownloadingDocx(true);
      await baixarDocxCronograma(cronogramaAtivo);
    } catch (err) {
      console.error('Falha ao baixar DOCX:', err);
      alert('Ocorreu um erro ao gerar o arquivo Word (.docx). Tente novamente.');
    } finally {
  ...
  return (
    <div className="min-h-screen bg-slate-100/70 text-slate-900 flex flex-col antialiased">
      ...
      <Navbar
        ...
        onBaixarDocx={handleBaixarDocxCompleto}
      ...
      {/* Banner de Sucesso Pós-Geração com Acesso Rápido ao DOCX */}
      {bannerGerado && (
        <div className="max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 pt-4">
          <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-sm animate-in fade-in slide-in-from-top-2 duration-200">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-bold text-slate-900 text-sm sm:text-base">
                  Roteiro e Briefing de {cronogramaAtivo.empresa} Gerados com Sucesso!
                </h3>
                <p className="text-xs text-slate-600 mt-0.5">
                  Cronograma para {cronogramaAtivo.mes_referencia} pronto com {cronogramaAtivo.total_conteudos} peças destrinchadas segundo os guardrails.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 self-end sm:self-center">
              <button
                onClick={handleBaixarDocxCompleto}
                disabled={isDownloadingDocx}
                className="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white text-xs sm:text-sm font-bold rounded-xl shadow-xs transition-all disabled:opacity-50"
              >
                {isDownloadingDocx ? (
                ...
                <span>Baixar DOCX do Briefing Completo</span>
              </button>
              <button
                onClick={() => setBannerGerado(false)}
                className="p-2 text-slate-400 hover:text-slate-600 rounded-lg hover:bg-emerald-100/60 transition-colors"
